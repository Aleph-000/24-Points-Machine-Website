...
static void wasm_append_line(const string& line) {
    if (g_wasm_limit > 0 && g_wasm_count >= g_wasm_limit) return;
    g_wasm_output += line;
...
EMSCRIPTEN_KEEPALIVE const char* hegel_solve(const char* line, int limit) {
    ...
    if (!line || !*line) return g_wasm_output.c_str();

    vector<Node> input = Solver::parse_nodes_from_line(string(line));
    if (input.empty()) return g_wasm_output.c_str();

    if (NO_NEGATIVE_INTERMEDIATE) {
        for (auto& nd : input) {
            if (nd.num.has_ll && nd.num.ll < 0) return g_wasm_output.c_str();
        }
    }
